...
export const clientApi = {
  ...
  getMessageCount: async (clientId: number | string) => {
    const response = await axiosInstance.get(`/admin/clients/${clientId}/message-count`);
    return response.data;
  },

  getMessageStats: async (clientId: number | string) => {
    const response = await axiosInstance.get(`/admin/clients/${clientId}/message-stats`);
    return response.data;
  },
  ...
  getClientLeads: async (clientId: number | string, params?: { q?: string; from?: string; to?: string; limit?: number; offset?: number; sort?: string }) => {
    const response = await axiosInstance.get(`/admin/clients/${clientId}/leads`, { params });
    return response.data;
  },
...
export const publicApi = {
  ...
  getAllBillingPlans: async () => {
    const response = await publicAxios.get('/billing/plans');
    return response.data;
  },

  getBillingPlanByCode: async (planCode: string) => {
    const response = await publicAxios.get(`/billing/plans/${planCode}`);
    return response.data;
  },
};
