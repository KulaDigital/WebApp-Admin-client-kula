...
import { authApi, clientApi, adminClientsApi } from '../../api';
...
const ClientSettings: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch detailed client info for widget config
        const clientId = clientRes?.client_id || meRes?.client_id;
        if (clientId) {
          try {
            const detailedRes = await adminClientsApi.getClient(clientId);
            const details = detailedRes?.client || detailedRes;
  ...
  const widgetConfig = clientDetails?.widget_config || widget;
    ...
    <div className="flex flex-col gap-6 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card" style={{ boxShadow: 'var(--shadow-md)' }}>
          ...
          <div className="card-body">
            <InfoRow label="Company Name">{clientDetails?.company_name || profile?.company_name || '—'}</InfoRow>
            <InfoRow label="Website URL">
              {clientDetails?.website_url || profile?.website_url ? (
                <a
                  href={clientDetails?.website_url || profile?.website_url}
                  target="_blank"
            ...
            <InfoRow label="Client ID">{clientDetails?.id ?? profile?.client_id ?? userRole?.clientId ?? '—'}</InfoRow>
          </div>
        </div>
        ...
        <div className="card" style={{ boxShadow: 'var(--shadow-md)' }}>
          ...
          <div className="card-body">
            <InfoRow label="Primary Color">
              {widgetConfig?.primary_color || widgetConfig?.primaryColor ? (
                <span className="inline-flex items-center gap-2">
                  <span className="inline-block w-4 h-4 rounded border" style={{ backgroundColor: widgetConfig?.primary_color || widgetConfig?.primaryColor, borderColor: 'var(--color-border)' }} />
                  <span className="font-mono text-small">{widgetConfig?.primary_color || widgetConfig?.primaryColor}</span>
                </span>
              ) : (
                '—'
              )}
            </InfoRow>
            <InfoRow label="Secondary Color">
              {widgetConfig?.secondary_color || widgetConfig?.secondaryColor ? (
                <span className="inline-flex items-center gap-2">
                  <span className="inline-block w-4 h-4 rounded border" style={{ backgroundColor: widgetConfig?.secondary_color || widgetConfig?.secondaryColor, borderColor: 'var(--color-border)' }} />
                  <span className="font-mono text-small">{widgetConfig?.secondary_color || widgetConfig?.secondaryColor}</span>
                </span>
              ) : (
                '—'
              )}
            </InfoRow>
            <InfoRow label="Position">{widgetConfig?.position ? capitalize(widgetConfig.position) : '—'}</InfoRow>
            <InfoRow label="Welcome Message">{widgetConfig?.welcome_message || widgetConfig?.welcomeMessage || '—'}</InfoRow>
          </div>
        </div>
      </div>
    </div>
