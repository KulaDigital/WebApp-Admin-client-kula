...
const MySubscription: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data: ClientProfile = await clientApi.getClientProfile();
        setSubscription(data.subscription || null);
        setUserEmail(data.email || '');
        setRequestEmail(data.email || '');
        
        // Extract client ID - the API returns it based on the authenticated user
        // For a client accessing their own data, we need to determine the client_id
        const id = data.client_id || (data as any).user_client_id;
        if (id) {
          setClientId(id);
        } else {
          console.warn('Client ID not found in response:', data);
          setMessageError('Unable to load message usage data');
        }
      } catch (err) {
  ...
  // Fetch message count when client ID is available
  useEffect(() => {
    if (clientId) {
      fetchMessageData();
    }
  ...
  // Fetch billing plan when subscription plan is available
  useEffect(() => {
    if (subscription?.plan) {
      fetchBillingPlanLimit();
    }
  }, [subscription?.plan]);
  ...
  const fetchMessageData = async () => {
    try {
      if (!clientId) {
        setMessageError('Client ID not available');
        return;
      }
      ...
      const errorMsg = err.response?.status === 404 
        ...
        : 'Failed to load message usage';
      setMessageError(errorMsg);
    }
  };

  const fetchBillingPlanLimit = async () => {
    try {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="card p-6 mb-6">
        ...
        {messageError ? (
          <p className="text-sm text-[var(--color-error)]">{messageError}</p>
        ) : messagesLimit > 0 ? (
          <ProgressBar label="Messages" used={messagesUsed} limit={messagesLimit} />
        ) : (
          <p className="text-sm text-[var(--color-text-secondary)]">Loading usage data...</p>
        )}
      </div>
      ...
      {/* D. Plan Details Comparison */}
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-[var(--color-text-primary)] mb-4" style={{ fontFamily: 'var(--font-heading)' }}>
          Available Plans
        </h2>
        <PlanDetails currentPlanCode={subscription?.plan?.toLowerCase()} />
      </div>
